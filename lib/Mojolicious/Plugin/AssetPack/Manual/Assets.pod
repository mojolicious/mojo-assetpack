...
The "some-asset.js" string is just so you can refere to the genrated bundle
in your templates L<later|Mojolicious::Plugin::AssetPack::Manual::Include>,
while the list of files after (foo.js and bar.js) are files that exists in
one of the public L<static|Mojolicious/static> directories defined in your
application.

=head1 Online assets

The assets does not need to be locally. They can also be
L<fetched|Mojolicious::Plugin::AssetPack/fetch> online:
...
The code aboove will download three assets from web and combine them with
