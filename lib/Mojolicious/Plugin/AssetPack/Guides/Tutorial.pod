...
The source/input files that make up a virtual asset can come from either...
...
TODO. (It should be possible to load dynamic source assets from the current
application)
...
In the example above we have defined a virtual asset named "app.css" which
...
define a source file. This means that the file above will result in the same as
in the L<example above|/Defining assets in the application>.

=head2 Using assets

Any processed asset can be accessed by refering to the virtual name.
