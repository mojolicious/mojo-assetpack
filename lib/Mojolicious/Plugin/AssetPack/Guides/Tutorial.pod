...
The plugin needs to be installed and loaded before any assets can be defined:
...
argument worth noting: "pipes". "pipes" needs to be an arrayref of all the pipes
you need to process your assets. Example:

  $app->plugin(AssetPack => {pipes => [qw(Sass Css Combine)]);

Loading the plugin with the arrayref above will enable AssetPack to process Sass
...
dependencies should be required only while developing, and not for running
the application. See L<https://github.com/jhthorsen/mojolicious-plugin-assetpack/blob/v2/cpanfile>
for a complete list, but here is the current list:
...
AssetPack does not do any heavy lifting itself: All of the processing is left
to the L<pipe objects|Mojolicious::Plugin::AssetPack::Pipe>.
...
Any file starting with "http" or "https" will be downloaded from the web using
...
Assets from the web will be cached locally to prevent downloading new and
untested assets on each application startup.
...
Assets should be defined when your application starts. This can either be done
using a L<definition file|Mojolicious::Plugin::AssetPack/process> or inside
your application.
...
useful when you have something like "sass/main.scss" that depends on
...
In production mode, the helper above will result in just one "link" tag. On the
other hand, if you are in "development" mode, it will result in one "link" tag
per source asset.
...
This again only requires external tools (less, coffee, ...) and modules
