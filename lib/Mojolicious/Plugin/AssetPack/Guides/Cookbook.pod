...
You might want to serve the assets from a domain differently from where the main
...
No cookies sent on each request. This is especially useful when you use
...
requests to one domain. With separate domains, static files can be loaded in
parallel.
