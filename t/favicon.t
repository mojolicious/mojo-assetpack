...
$t->app->asset->process('favicon.ico' => 'image/master_favicon_thumbnail.json', 'image/master_favicon_thumbnail.png');
...
$t->get_ok('/')->status_is(200)->element_exists('[href$="114x114.png"][rel="apple-touch-icon"][sizes="114x114"]')
  ...
  ->element_exists('[name="msapplication-TileColor"][content="#456789"]');
...
$t->tx->res->dom->find("[href], [content]")->each(sub {
  ...
  return unless $src =~ m!^/!;    # Skip colors and application names
...

{
  ...
  local *Mojolicious::Plugin::AssetPack::Pipe::Favicon::_fetch_assets = sub { die $_[1] };
  eval {
    $t->app->asset->process('favicon.cool_beans.ico' => '/image/master_favicon_thumbnail.json', '/image/sample.png');
  };
